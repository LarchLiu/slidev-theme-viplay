...
export function audioSrc(config: SubtitlesConfig, word: string) {
  ...
  // return `/${hash}`
  let apiCustome = ''
  for (const key in config.apiCustome)
    apiCustome += `&${key}=${config.apiCustome[key]}`
  const url = `${config.ttsApi}?format=${config.ext}&text=${word}&model=${currentTTSLang.value}_${currentTTSModel.value}${apiCustome}`
  return url
}
